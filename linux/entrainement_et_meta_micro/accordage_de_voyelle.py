                    ...
                    # Prendre les k plus proches dans le rayon
                        ...
                        if predictions_labels in letters:
                            ...
                            if current_time - last_update_time >= 0.01:
                                ...
                                # Envoyer dans la queue pour le thread principal
                                graph_queue.put({
                                    "displayed_letters": displayed_letters,
                                    "needle_pos": (x_tip, y_tip),
                                    "history": "".join(historique_lettres),
                                })
...
def update_graph():
    try:
        while not graph_queue.empty():
            update = graph_queue.get_nowait()
            # Mettre à jour les lettres
            for i, txt in enumerate(text_objects):
                txt.set_text(update["displayed_letters"][i])
            # Mettre à jour l’aiguille
            x_tip, y_tip = update["needle_pos"]
            needle.set_data([0, update["needle_pos"][0]], [-1.2, update["needle_pos"][1]])
            # Mettre à jour l’historique
            history_text.set_text(update["history"])
    ...
    # Replanifier l'appel en fonction du backend
    ...
    if "qt" in backend:
        ...
        fig.canvas.manager.window.after(20, update_graph)
    else:
        # Sinon boucle en brut (moins élégant mais fonctionne)
        plt.pause(0.02)
        if not stop_flag.is_set():
            update_graph()

# FLAG pour arrêter proprement la boucle audio
stop_flag = threading.Event()

def on_close(event):
    print("Fermeture de la fenêtre : arrêt en cours...")
    stop_flag.set()
...
# Lancer la mise à jour graphique
update_graph()

# Bloquer le main thread sur plt.show
plt.show(block=True)
